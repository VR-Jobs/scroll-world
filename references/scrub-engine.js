/* ============================================================================
   ...
   Served pages normally load each clip as a Blob (always seekable) and scrub currentTime,
   so they do not depend on HTTP byte-range support. A file:// page, or a config with
   `directVideo: true`, assigns the local MP4 directly because file-origin fetch is blocked.
   ========================================================================== */
...
function mountScrollWorld(container, config) {
  ...
  // A directly opened local HTML file cannot reliably fetch sibling files because
  // browsers give file:// documents an opaque origin. In that mode use the MP4 as
  // the video src directly; local files are already fully seekable. Served pages
  // keep the Blob path so they do not depend on HTTP Range support.
  const directVideoMode = window.location.protocol === 'file:' || config.directVideo === true;
  ...
  SEGMENTS.forEach(s => {
    ...
    s.el = scene; s.img = img; s.video = null; s.objectUrl = null; s.hasClip = false;
  ...
  function enterStillsMode() {
    ...
    SEGMENTS.forEach(s => {
      ...
      s.video = null; s.objectUrl = null; s.hasClip = false; s.ready = false; s.loading = false;
    });
    read();
  }
  ...
  function loadClip(s) {
    ...
    const attachVideo = (src, objectUrl) => {
      const v = document.createElement('video');
      v.className = 'sw-scene__video';
      v.muted = true; v.playsInline = true; v.preload = 'auto';
      v.setAttribute('muted', ''); v.setAttribute('playsinline', '');
      v.src = src;
      s.objectUrl = objectUrl || null;
      ...
      v.addEventListener('error', () => {
        ...
        v.remove();
        s.video = null; s.objectUrl = null; s.hasClip = false; s.ready = false; s.loading = false;
        console.warn('scroll-world: unable to load local clip', url);
      }, { once: true });
      s.el.appendChild(v); s.video = v; s.hasClip = true;
    };

    if (directVideoMode) {
      attachVideo(new URL(url, document.baseURI).href, null);
      return;
    }
    ...
    fetch(url).then(r => r.ok ? r.blob() : Promise.reject(new Error('404')))
      .then(blob => {
        ...
        attachVideo(objectUrl, objectUrl);
